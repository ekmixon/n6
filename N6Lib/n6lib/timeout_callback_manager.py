...
from n6lib.concurrency_helpers import protected_from_concurrency
...
CONCURRENCY_PROTECTION_OPTIONS = dict(
    mutex_key='n6lib.timeout_callback_manager.TimeoutCallbackManager',
...
class TimeoutCallbackManager(object):
    ...
    * Because callbacks are called asynchronously, any exception raised
    ...
    def ensure_preparations_and_monkey_patching_done():
        # should called very early (e.g., in `n6lib`'s `__init__.py` )
        ...
        def _custom_n6lib_getsignal_wrapper(signalnum):
        ...
        def _custom_n6lib_signal_wrapper(signalnum, handler):
            with alarm_setting_funcs_mutex:
                if signalnum == _SIGALRM:
        ...
        def _custom_n6lib_alarm_wrapper(*args, **kwargs):
            with alarm_setting_funcs_mutex:
                if activated_instances:
                    raise RuntimeError(
        ...
        def _custom_n6lib_setitimer_wrapper(which, *args, **kwargs):
            with alarm_setting_funcs_mutex:
        ...
        if (getattr(orig_getsignal_func, '__name__', None)
                == _custom_n6lib_getsignal_wrapper.__name__):
            raise RuntimeError(
                'something strange: signal.getsignal seems to '
                'be already set to _custom_n6lib_getsignal_wrapper!')

        if (getattr(orig_signal_func, '__name__', None)
                == _custom_n6lib_signal_wrapper.__name__):
            raise RuntimeError(
                'something strange: signal.signal seems to '
                'be already set to _custom_n6lib_signal_wrapper!')

        if (getattr(orig_alarm_func, '__name__', None)
                == _custom_n6lib_alarm_wrapper.__name__):
            raise RuntimeError(
                'something strange: signal.alarm seems to '
                'be already set to _custom_n6lib_alarm_wrapper!')

        if (getattr(orig_setitimer_func, '__name__', None)
                == _custom_n6lib_setitimer_wrapper.__name__):
            raise RuntimeError(
                'something strange: signal.setitimer seems to '
                'be already set to _custom_n6lib_setitimer_wrapper!')

        # actual monkey patching + completing necessary class attributes:

        signal.getsignal = _custom_n6lib_getsignal_wrapper
        signal.signal = _custom_n6lib_signal_wrapper
        signal.alarm = _custom_n6lib_alarm_wrapper
        signal.setitimer = _custom_n6lib_setitimer_wrapper
