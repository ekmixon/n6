...
class _MockerMixin(RequestHelperMixin, DBConnectionPatchMixin):
    ...
    def _get_mocked_db_state(self):
        ...
        return {
            ...
            'component': [
                regular_comp,
                privileged_comp,
                comp_whose_login_is_illegal_username,
            ],
    ...
    def assertConnectorUsedOnlyAfterEnsuredClean(self):
        if first_two_connector_uses := self.connector_mock.mock_calls[:2]:
            # noinspection PyUnresolvedReferences
...
def foreach_username(seq_of_usernames):
    seq_of_params = [
        param(username=username).label(f'u:{username}')
...
class TestResourceView(_N6BrokerViewTestingMixin, unittest.TestCase):
    ...
    @paramseq
    def __resource_types(self):
        yield param(resource=EXCHANGE).label('ex')
        yield param(resource=QUEUE).label('qu')

    @paramseq
    def __illegal_resource_types(self):
        yield param(resource=TOPIC)
        yield param(resource='whatever')
        yield param(resource='')

    @paramseq
    def __permission_levels(self):
        yield param(permission=CONFIGURE).label('c')
        yield param(permission=WRITE).label('w')
        yield param(permission=READ).label('r')

    @paramseq
    def __illegal_permission_levels(self):
        yield param(permission='whatever')
        yield param(permission='')

    @paramseq
    def __some_autogenerated_queue_names(self):
        yield param(name=f'{AUTOGENERATED_QUEUE_PREFIX}.queue1')
        yield param(name=f'{AUTOGENERATED_QUEUE_PREFIX}-some_other_queue')
        yield param(name=f'{AUTOGENERATED_QUEUE_PREFIX}#$%#$afdiajsdfsadwe33')
        yield param(name=AUTOGENERATED_QUEUE_PREFIX)

    @paramseq
    def __some_not_autogenerated_queue_names(self):
    ...
    @paramseq
    def __various_nonpush_exchange_names(self):
    ...
    @paramseq
    def __matching_knownuser_exchange_pairs(self):
    ...
    @paramseq
    def __not_matching_regularuser_exchange_pairs(self):
...
class TestTopicView(_N6BrokerViewTestingMixin, unittest.TestCase):
    ...
    @paramseq
    def __various_exchange_names(self):
        yield param(name=ORG1)
        yield param(name=ORG2)
